...
There is a Jupyter notebook [time_signature_rnn.ipynb](https://github.com/tensorflow/magenta-demos/blob/master/jupyter-notebooks/time_signature_rnn.ipynb)
...
time_signature_rnn_generate \
--config=${CONFIG} \
--bundle_file=${BUNDLE_PATH} \
--output_dir=/tmp/time_signature_rnn/generated \
--num_outputs=10 \
--num_steps=3000 \
...
For a full list of command line options, run `time_signature_rnn_generate --help`.
...
--output_dir=/tmp/time_signature_rnn/sequence_examples \
--eval_ratio=0.10
```
...
```
time_signature_rnn_train \
--config=${CONFIG} \
--run_dir=/tmp/time_signature_rnn/logdir/run1 \
--sequence_example_file=/tmp/time_signature_rnn/sequence_examples/training_performances.tfrecord
```
...
--config=${CONFIG} \
--run_dir=/tmp/time_signature_rnn/logdir/run1 \
--sequence_example_file=/tmp/time_signature_rnn/sequence_examples/eval_performances.tfrecord \
--eval
```
...
```
tensorboard --logdir=/tmp/time_signature_rnn/logdir
```
...
`--run_dir` should be the same directory used for the training job. The `train` subdirectory within `--run_dir` is where the latest checkpoint file will be loaded from. For example, if we use `--run_dir=/tmp/time_signature_rnn/logdir/run1`. The most recent checkpoint file in `/tmp/time_signature_rnn/logdir/run1/train` will be used.
...
time_signature_rnn_generate \
  --config=${CONFIG}
  --run_dir=/tmp/time_signature_rnn/logdir/run1 \
  --bundle_file=/tmp/time_signature_rnn.mag \
