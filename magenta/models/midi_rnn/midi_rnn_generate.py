...
"""Generate polyphonic MIDI sequences from a trained checkpoint.
...
from magenta.models.midi_rnn import midi_rnn_sequence_generator
...
tf.app.flags.DEFINE_string(
    'config', 'midi', 'Config to use.')
tf.app.flags.DEFINE_string(
    'output_dir', '/tmp/midi_rnn/generated',
...
tf.app.flags.DEFINE_integer(
    ...
    'track length + generated steps. Each step is 1/24 of a quarter note.')
tf.app.flags.DEFINE_string(
    'primer_pitches', '',
...
def run_with_flags(generator):
  """Generates MIDI sequence tracks and saves them as MIDI files.

  Uses the options specified by the flags defined in this module.

  Args:
    generator: The MidiRnnSequenceGenerator to use for generation.
  """
...
def main(unused_argv):
  ...
  config = midi_rnn_model.default_configs[config_id]
  config.hparams.parse(FLAGS.hparams)
  # Having too large of a batch size will slow generation down unnecessarily.
  config.hparams.batch_size = min(
      config.hparams.batch_size, FLAGS.beam_size * FLAGS.branch_factor)

  generator = midi_rnn_sequence_generator.MidiRnnSequenceGenerator(
      model=midi_rnn_model.MidiRnnModel(config),
