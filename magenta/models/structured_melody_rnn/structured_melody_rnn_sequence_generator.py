...
class StructuredMelodyRnnSequenceGenerator(mm.BaseSequenceGenerator):
  ...
  def __init__(self, model, details, steps_per_quarter=4, checkpoint=None,
               bundle=None):
    """Creates a StructuredMelodyRnnSequenceGenerator.

    Args:
      model: Instance of StructuredMelodyRnnModel.
    ...
    """
    super(StructuredMelodyRnnSequenceGenerator, self).__init__(
...
def get_generator_map():
  ...
  def create_sequence_generator(config, **kwargs):
    return StructuredMelodyRnnSequenceGenerator(
        structured_melody_rnn_model.StructuredMelodyRnnModel(config), config.details,
        steps_per_quarter=config.steps_per_quarter, **kwargs)

  return {key: partial(create_sequence_generator, config)
          for (key, config) in structured_melody_rnn_model.default_configs.items()}
