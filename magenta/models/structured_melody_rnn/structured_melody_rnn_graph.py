...
def extract_input_windows(inputs, batch_size, input_size, window_size):
  ...
  The number of windows extracted will be the same as the sequence length, and
  the window for each step will end with that step. The beginning of the
  sequence will be padded with zeros.
  ...
  Returns:
    A tensor with shape `[batch_size, num_steps, input_size, window_size]`
    containing the input window at each step.
  """
  padded_inputs = tf.expand_dims(
      tf.pad(inputs, [[0, 0], [window_size - 1, 0], [0, 0]]), -1)
...
def build_graph(mode, config, sequence_example_file_paths=None):
  ...
  with tf.Graph().as_default() as graph:
    ...
    input_windows = extract_input_windows(
        inputs, hparams.batch_size, input_size, hparams.window_size)

    # Encode input windows and compute self-similarity.
