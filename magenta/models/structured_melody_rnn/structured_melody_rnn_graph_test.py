...
"""Tests for structured_melody_rnn_rnn_graph."""
...
class StructuredMelodyRnnGraphTest(tf.test.TestCase):

  def setUp(self):
    self._sequence_file = tempfile.NamedTemporaryFile(
        prefix='StructuredMelodyRnnGraphTest')
  ...
  def testBuildTrainGraph(self):
    g = structured_melody_rnn_graph.build_graph(
        'train', self.config,
  ...
  def testBuildGenerateGraph(self):
    g = structured_melody_rnn_graph.build_graph('generate', self.config)
    self.assertTrue(isinstance(g, tf.Graph))
