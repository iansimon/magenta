...
"""Train and evaluate a structured melody RNN model."""
...
tf.app.flags.DEFINE_string('run_dir', '/tmp/structured_melody_rnn/logdir/run1',
                           ...
                           'your runs.')
tf.app.flags.DEFINE_string('config', 'self_similarity_rnn',
                           'The config to use.')
...
def main(unused_argv):
  ...
  config = structured_melody_rnn_model.default_configs[FLAGS.config]

  mode = 'eval' if FLAGS.eval else 'train'
  graph = structured_melody_rnn_graph.build_graph(
