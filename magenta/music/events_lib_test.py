...
class DummyEventSequence(events_lib.SimpleEventSequence):

  def __init__(self, **kwargs):
    super(DummyEventSequence, self).__init__(pad_event=0, **kwargs)
...
class EventsLibTest(tf.test.TestCase):
  ...
  def testDeepcopy(self):
    events = DummyEventSequence(
        events=[0, 1, 2], start_step=0, steps_per_quarter=4, steps_per_bar=8)
  ...
  def testSetLength(self):
    events = DummyEventSequence(events=[60], start_step=9)
    events.set_length(5)
    ...
    events = DummyEventSequence(events=[60, 0, 0, 0])
    events.set_length(3)
  ...
  def testIncreaseResolution(self):
    events = DummyEventSequence(events=[1, 0, 1, 0], start_step=5,
                                steps_per_bar=4, steps_per_quarter=1)
    ...
    events = DummyEventSequence(events=[1, 0, 1, 0])
